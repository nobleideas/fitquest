...
class HomePageState extends State<HomePage> with WidgetsBindingObserver {
  ...
  Future<void> _openSharePicker() async {
    final entries = _workoutEntries().where((s) => s.isCurrentUser).toList();
  ...
  List<DayWorkoutSummary> _buildFriendWorkoutSummaries(
    ...
    for (final entry in byDay.entries) {
      ...
      for (final row in rows) {
        ...
        final volume =
            _numToDouble(row['weight']) * _numToInt(row['reps']);
  ...
  Future<void> _loadRecentExercises() async {
    ...
    try {
      ...
      final friendFeed = await _friendProfileService.loadAcceptedFriendWorkoutFeed();
